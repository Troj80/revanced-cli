...
class AdbProtocolHandler implements Runnable {
    ...
    private void sync(DataOutput output, DataInput input) throws IOException {
        ...
        try {
            ...
            switch (id) {
                case "SEND" -> syncSend(output, input, length);
                case "RECV" -> syncRecv(output, input, length);
                case "LIST" -> syncList(output, input, length);
                default -> throw new JadbException("Unknown sync id " + id);
            }
