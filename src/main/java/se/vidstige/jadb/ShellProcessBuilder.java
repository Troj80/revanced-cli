...
public class ShellProcessBuilder {
    ...
    private OutputStream getOutputStream(OutputStream os, ProcessBuilder.Redirect destination, OutputStream inherit) throws IOException {
        ...
        return switch (destination.type()) {
            case PIPE -> new PipedOutputStream();
            case INHERIT -> inherit;
            case READ -> throw new IllegalArgumentException("Redirect invalid for writing: " + destination);
            case WRITE -> Files.newOutputStream(destination.file().toPath());
            case APPEND -> Files.newOutputStream(destination.file().toPath(), StandardOpenOption.CREATE, StandardOpenOption.APPEND, StandardOpenOption.WRITE);
            default -> throw new IllegalArgumentException("Unknown redirect type: " + destination);
        };
    }
